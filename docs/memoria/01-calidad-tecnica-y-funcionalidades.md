...
A efectos de especificacion de componentes SaaS y ubicacion de despliegue, se distinguen los siguientes elementos: (1) **Componentes operados como SaaS por el adjudicatario**: la aplicacion web del canal ciudadano y del panel de administracion, la API backend (NestJS), la base de datos PostgreSQL que alberga contenidos, fuentes, conversaciones, auditoria y configuracion, y la capa RAG e ingesta de conocimiento se despliegan y operan de forma integrada por el adjudicatario como parte del servicio SaaS contratado. (2) **Ubicacion del despliegue**: dichos componentes se desplegaran en infraestructura cloud ubicada en territorio nacional o en la Union Europea, con preferencia por centros de datos en Espana (por ejemplo regiones Azure España, AWS eu-south-1 o equivalentes GCP/proveedores nacionales), garantizando el cumplimiento normativo en materia de datos y la preferencia del pliego por territorio nacional. (3) **Servicios SaaS de terceros**: el proveedor de avatar (Anam) y, en su caso, el proveedor de modelos de lenguaje (OpenAI u otro equivalente) son servicios externos consumidos por la plataforma; sus condiciones de hosting y tratamiento de datos son las establecidas por cada proveedor, pudiendo evaluarse alternativas con hosting en UE cuando la contratacion lo requiera.
